/* Wallet connection for Robinhood Chain mainnet. Discovers injected wallets through EIP-6963
   ...
   switches or adds chain 4663 and exposes viem clients. Loaded as an ES module by Rouri. */
...
function onAccounts(a) { W.address = a && a[0] ? a[0] : null; if (!W.address) { try { localStorage.removeItem('rouri:wallet'); } catch {} } emit(); }
...
export async function connect(id) {
  ...
  try { localStorage.setItem('rouri:wallet', d ? d.info.rdns : 'injected'); } catch {}
...
export async function restore() {
  let rdns = null; try { rdns = localStorage.getItem('rouri:wallet'); } catch {}
...
export function disconnect() {
  W.address = null; try { localStorage.removeItem('rouri:wallet'); } catch {}
...
/* raw calldata (Uniswap v4 PositionManager actions): simulated with eth_call first, then sent */
export async function sendRaw({ to, data, value = 0n }) {
  if (!W.address) throw Error('Connect a wallet first.');
  if (W.chainId !== CHAIN.id) await switchChain();
  await pub.call({ account: W.address, to, data, value });
  const hash = await W.client.sendTransaction({ account: W.address, to, data, value });
  return { hash, wait: () => pub.waitForTransactionReceipt({ hash }) };
}
